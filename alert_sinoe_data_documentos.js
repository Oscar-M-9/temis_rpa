...
const logger = winston.createLogger({
    ...
    //   new winston.transports.File({ filename: 'logs/alert_sinoe_data_documentos.log' }) // Registra en un archivo
      new winston.transports.File({ filename: '/home/temisperu/public_html/rpa.temisperu.com/RPA/logs/alert_sinoe_data_documentos.log' }) // Registra en un archivo
    ],
});
...
async function obtenerPrimerRegistro(conexion) {
    return new Promise((resolve, reject) => {
        conexion.query('SELECT * FROM temp_document_presentados WHERE estado = ? LIMIT 1', ['pendiente'], async (error, results) => {
            if (error) {
                reject(error);
            } else if (results.length === 0) {
                // No hay registros, restablecer la secuencia de autoincremento a 0
                await resetAutoIncrement(conexion, 'temp_document_presentados');
                resolve(null);
            } else {
                resolve(results[0]);
            }
...
async function enviarDatosAlEndpoint(data) {
    const endpointURL = urlAPI + '/sinoe-historial-data';
...
async function eliminarTempRegistro(conexion, id) {
    try {
        const sql = 'DELETE FROM temp_document_presentados WHERE id = ?';
