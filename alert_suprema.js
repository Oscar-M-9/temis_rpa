...
// Crear una función para enviar los datos a un endpoint que actualizaria 
// los movimientos y enviaria el correo electronico
async function enviarDatosAlEndpointUpdateMail(data, urlPage) {
    const endpointURL = urlPage + '/api/update-movements-suprema';
    const jsonData = JSON.stringify(data);
...
async function obtenerUserParte(conexion, id) {
    return new Promise((resolve, reject) => {
        conexion.query('SELECT * FROM user_partes WHERE id_exp = ? AND entidad = ?', [id, 'suprema'], (error, results) => {
            if (error) {
                reject(error);
            } else if (results.length === 0) {
                resolve(null); // No hay registros
...
async function main() {
    try {
        ...
        if (firstExp) {
            ...
            await enviarDatosAlEndpointUpdateMail(data2, urlPage);
            ...
            //                 const insertResult = await ejecutarQueryInsert(conexion, sqlInsertM, dataInsertM);
            ...
            //                     fechaHoraActual
            //                 ];
            //                 await ejecutarQueryInsert(conexion, sqlInsertHistoryMovements, valuesHistoryMovements);
