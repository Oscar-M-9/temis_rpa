...
const logger = winston.createLogger({
    ...
    transports: [
        new winston.transports.File({
            // filename: 'logs/alert_client.log',
            filename: '/home/temisperu/public_html/rpa.temisperu.com/RPA/logs/alert_client.log',
            level: 'info',
        }),
...
async function obtenerRegistrosTask(conexion) {
    ...
    return new Promise((resolve, reject) => {
        conexion.query('SELECT * FROM alert_clientes WHERE  CURDATE() <= fecha_limite', (error, results) => {
            if (error) {
                reject(error);
...
async function obtenerUserParte(conexion, codeUser, codeCompany) {
    return new Promise((resolve, reject) => {
        conexion.query('SELECT * FROM users WHERE code_user = ? AND code_company = ?', [codeUser, codeCompany], (error, results) => {
            if (error) {
                reject(error);
            } else if (results.length === 0) {
                resolve(null); // No hay registros
    ...
    try {
        ...
        for (const row in firstRecord) {
            if (Object.hasOwnProperty.call(firstRecord, row)) {
                ...
                if (!resultEmails){
                    const fechaYHora = new Date().toUTCString();
                    strMsg = fechaYHora + ': No se encontró correo id alert_client (' + element.id + ') id_cliente:'+firstClient.id+' '+ resultEmails;
                    logger.error(strMsg);
                    conexion.end();
                    return;
                }
